...
//
// Architecture: #three-canvas (WebGL) sits BELOW #game-canvas (Canvas 2D).
// Tile colours are computed in JS (MeshBasicMaterial) — this sidesteps the
// Three.js r128 bug where MeshLambertMaterial + InstancedMesh.setColorAt()
// never enables USE_INSTANCING_COLOR in the vertex-shader compile path, making
// all Lambert tiles invisible. MeshBasicMaterial + manual torch maths = reliable.
...
// ── Mist ──────────────────────────────────────────────────────────────────────
let _mistPoints = null, _mistPos = null, _mistVel = null;
const _MIST_COUNT = 220;

// ── Ambient state (set per-frame by _updateAmbient) ──────────────────────────
// These drive the manual per-tile lighting instead of Three.js light objects.
let _ambR = 0.55, _ambG = 0.42, _ambB = 0.28; // current ambient tint
...
// ── Rarity → glow colour ─────────────────────────────────────────────────────
const _RARITY_COL = {
    common:    0xffd8a0, uncommon: 0x50e870,
    rare:      0x4090ff, epic:     0xc060ff,
    legendary: 0xffa020, mythic:   0xff30a0,
};
...
function initThreeJS() {
    ...
    _renderer = new THREE.WebGLRenderer({ canvas: gc, antialias: false, alpha: false,
    ...
    // Keep PointLights in scene — they illuminate nothing right now but are here
    // for future MeshStandard objects (particles, effects meshes, etc.)
    _torchLight  = new THREE.PointLight(0xff9820, 4.5, 700, 1.4);
    _torchLight.position.z = 44;  _scene.add(_torchLight);
    _torchLight2 = new THREE.PointLight(0xffb840, 1.8, 350, 1.8);
    _torchLight2.position.z = 22; _scene.add(_torchLight2);
    ...
    _THREE_ACTIVE = true;
    console.log('[render-three] WebGL active — MeshBasicMaterial tiles with JS lighting');
    return true;
}
...
function _buildTileMeshes() {
    ...
    // This is the ONLY reliable path for InstancedMesh colours in Three.js r128.
...
// ── Manual per-tile lighting ──────────────────────────────────────────────────
// Replaces Three.js Lambert shader lighting.
// Returns a lit [r,g,b] clamped to [0,1] for a tile at (wx,wy).
...
function updateThreeDungeon() {
    ...
    // Update ambient state first so _litColor uses current values
    _updateAmbient(t);
    ...
    const p = s.player;
    const px = p ? ((p.renderX !== undefined ? p.renderX : p.x * TS) + TS / 2) : -9999;
    const py = p ? -((p.renderY !== undefined ? p.renderY : p.y * TS) + TS / 2) : -9999;

    // Torch flicker (reuse for _litColor)
    _torchFlicker = 1.0 + Math.sin(t * 3.8) * 0.12 + Math.sin(t * 7.1) * 0.05;
        ...
        for (let tx = 0; tx < mapW; tx++) {
            ...
            if (tile === 1) {
                ...
                if (fi < _MAX_INST) {
                    ...
                    const pal = tile === 2 ? PAL.FLOOR_EXIT
                              : tile === 3 ? PAL.FLOOR_UP
                              : PAL.FLOOR;
            ...
            // Fog of war: dark overlay for unrevealed tiles.
    ...
    if (p && _torchLight) {
        _torchLight.position.set(px, py, 44);
        _torchLight2.position.set(px + 10, py - 8, 24);
        _torchLight.intensity  = 5.0 * _torchFlicker;
...
// ── Ambient state update (drives _ambR/G/B used in _litColor) ────────────────
function _updateAmbient(t) {
    const s = gameState;
    const floor = s?.floor || 0;
    const p = s?.player;
    const hpFrac = p ? p.hp / Math.max(1, p.maxHp) : 1;
    const bossNear = (s?.enemies || []).some(
        e => e.hp > 0 && (e.type === 'boss' || e.bossVariant) && s.revealed?.[e.y]?.[e.x]);

    // Depth palette: warm shallow → blue-grey mid → purple deep
    let r, g, b;
    if      (floor === 0)  { r = 0.55; g = 0.42; b = 0.28; }
    else if (floor < 15)   { r = 0.48; g = 0.36; b = 0.22; }
    else if (floor < 35)   { r = 0.30; g = 0.34; b = 0.44; }
    else if (floor < 65)   { r = 0.26; g = 0.20; b = 0.44; }
    else                   { r = 0.30; g = 0.12; b = 0.44; }
...
function _updateSconces(t) {
    const s = gameState;
    if (!s?.dungeon || s.floor === 0) { _sconceLights.forEach(l => l.visible = false); return; }
